                ...
                {CONST.coup.map((item, i) =>
                    <View key={i} style={{ flexDirection: 'row', width: WP(85), alignSelf: 'center', borderWidth: 2, borderColor: palette.maalta, marginVertical: HP(2) }}>
                        <View style={{paddingVertical:HP(2)}}>
                            <Text style={{ color: palette.maalta,lineHeight:14, fontFamily: fontFamily.semi_bold, fontSize: 16 }}>{item.offer}</Text>
                            <Text style={{fontFamily:fontFamily.regular,fontSize:12,lineHeight:14}}>有效日期：{item.date}  </Text>
                        </View>
                    </View>
                )}
            </View>
